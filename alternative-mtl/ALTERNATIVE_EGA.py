...
from utils.proposed_MTL.alternate_EGA_generator import DataGenerator
from utils.confusion_MTL.confusion_EGA_generator import DataGenerator as Test_DataGenerator
...
def main():
    ...
    if MODEL == 'vggFace':
        ...
        model = Net(MODEL,1,4,emotion_classes,pose_classes,age_classes,gender_classes,args.is_dropout,args.is_bn,args.weights_decay)
        MODEL = model.name
        if args.is_freezing:
    ...
    main_path='{}-{}/{}/'.format(EMOTION_DATASET,GENDER_AGE_DATASET,MODEL)
    ...
    model_names = weights_path +'.{epoch:02d}-{val_emotion_prediction_my_acc:.4f}-{val_gender_prediction_my_acc:.4f}-{val_age_prediction_my_acc:.4f}.hdf5'
    ...
    if not args.is_freezing:
        ...
        for epoch in  range(EPOCH):
            if epoch==0:
                print('initializing')
                model.fit_generator(
                Test_DataGenerator(model,None,train_emotion_paths,train_gender_age_paths,train_emotion,train_gender,train_age,args.batch_size,args.is_distilled,args.is_pesudo,args.is_interpolation,args.selection_threshold,args.interpolation_weights,is_augmentation=args.is_augmentation),
                validation_data = Test_DataGenerator(model, None,test_emotion_paths,test_gender_age_paths, test_emotion,test_gender,test_age,args.batch_size,args.is_distilled,args.is_pesudo,args.is_interpolation,args.selection_threshold,args.interpolation_weights,is_augmentation=False),
                epochs=1,
                verbose=2,
                workers=1,
                ...
                if epoch%2==0:
                    is_emotion=True
                    if args.is_naive:
                        model.compile(optimizer=adam, loss=loss_dict,loss_weights=[1,0,0],metrics=metrics_dict)
                else:
                    is_emotion=False
                    if args.is_naive:
                        model.compile(optimizer=adam, loss=loss_dict,loss_weights=[0,1,1],metrics=metrics_dict)
                model.fit_generator(
                    DataGenerator(model,model_pre,train_emotion_paths,train_gender_age_paths,train_emotion,train_gender,train_age,args.batch_size,is_emotion,args.is_distilled,args.is_pesudo,args.is_interpolation,args.selection_threshold,args.interpolation_weights,is_augmentation=args.is_augmentation),
                    validation_data = Test_DataGenerator(model, None,test_emotion_paths,test_gender_age_paths, test_emotion,test_gender,test_age,args.batch_size,args.is_distilled,args.is_pesudo,args.is_interpolation,args.selection_threshold,args.interpolation_weights,is_augmentation=False),
                    epochs=1,
                    verbose=2,
                    workers=1,
