...
from utils.proposed_MTL.alternate_EA_generator import DataGenerator
from utils.confusion_MTL.confusion_EA_generator import DataGenerator as Test_DataGenerator
from utils.callback import DecayLearningRate
from utils.pseudo_density_distribution.pseudo_label_weights_EA import assign_weights
...
# def base_STL_model(MODEL,is_EP,emotion_path,age_path,age_path,emotion_classes,age_classes,age_classes,gender_classes):
#     emotion_model=Net(MODEL,1,0,emotion_classes,age_classes,age_classes,gender_classes)
#     age_model=Net(MODEL,1,5,emotion_classes,age_classes,age_classes,gender_classes)
#     age_model=Net(MODEL,1,1,emotion_classes,age_classes,age_classes,gender_classes)
#     emotion_model.load_weights(emotion_path)
#     age_model.load_weights(age_path)
#     age_model.load_weights(age_path)
#     if is_EP:
#         model= Net(MODEL,1,11,emotion_classes,age_classes,age_classes,gender_classes)
#     else:
#         model= Net(MODEL,1,9,emotion_classes,age_classes,age_classes,gender_classes)
...
#         model.layers[24].set_weights(age_model.layers[22].get_weights())
#         return model
#     else:
...
def main():
    ...
    model_names = weights_path +'.{val_emotion_prediction_my_acc:.4f}-{val_age_prediction_my_acc:.4f}.hdf5'
    ...
    if not args.is_freezing:
        ...
        for epoch in  range(EPOCH):
            if epoch==0:
                print('initializing')
                model.fit_generator(
                Test_DataGenerator(model,None,train_emotion_paths,train_age_paths,train_emotion,train_age,args.batch_size,args.is_distilled,args.is_pesudo,args.is_interpolation,args.selection_threshold,args.interpolation_weights,args.is_augmentation,None,None),
                validation_data = Test_DataGenerator(model, None,test_emotion_paths,test_age_paths, test_emotion,test_age,args.batch_size,args.is_distilled,args.is_pesudo,args.is_interpolation,args.selection_threshold,args.interpolation_weights,False,None,None),
                epochs=1,
                verbose=2,
                workers=1,
                ...
                if args.is_pesudo or args.is_interpolation or args.is_distilled:
                    ...
                    pesudo_data = assign_weights(model,is_emotion,train_emotion_paths,train_emotion,train_age_paths,train_age,args.cluster_k,args.is_augmentation,args.selection_threshold,args.is_distilled,args.is_pesudo_confidence,args.is_pesudo_density,args.is_pesudo_distribution,is_hard_weight,args.interpolation_weights,args.distill_t,args.density_t)
                else:
                    pesudo_data=None
                model.fit_generator(
                    DataGenerator(model,model_pre,train_emotion_paths,train_age_paths,train_emotion,train_age,args.batch_size,is_emotion,args.is_distilled,args.is_pesudo,args.is_interpolation,args.selection_threshold,args.interpolation_weights,args.is_augmentation,pesudo_data),
                    validation_data = Test_DataGenerator(model, None,test_emotion_paths,test_age_paths, test_emotion,test_age,args.batch_size,args.is_distilled,args.is_pesudo,args.is_interpolation,args.selection_threshold,args.interpolation_weights,False,None,None),
