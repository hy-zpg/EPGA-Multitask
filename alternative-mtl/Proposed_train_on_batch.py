...
parser.add_argument('--dataset_emotion',
                    choices=['ferplus','ferplus','sfew','expw'],
                    default='expw',
...
def path_file(model,EMOTION_DATASET,GENDER_AGE_DATASET,MODEL):
    logs_path = './train_logs/PROPOSED/{}/{}-{}/{}/'.format(model.task_type,EMOTION_DATASET,GENDER_AGE_DATASET,MODEL)
    weights_path = './train_weights/PROPOSED/{}/{}-{}/{}/'.format(model.task_type,EMOTION_DATASET,GENDER_AGE_DATASET,MODEL)
...
def store_model_choice(model,EMOTION,GENDER,AGE,emotion_acc,gender_acc,age_acc,store_path,epoch):
    ...
    if model.task_type==4:
        ...
        if (dis1+dis2+dis3)<1:
            ...
            acc = str(emotion_acc)+'-'+'-'+str(age_acc)
            model.save(store_path+str(epoch)+'_'+acc+'.hdf5')
            return emotion_acc,gender_acc,age_acc
...
def test_model(model,callback,emotion_valid_name,gender_valid_name,age_valid_name,emotion_generater,gender_age_generater,BATCH_NB_TEST_1,BATCH_NB_TEST_2,EMOTION_ACC,GENDER_ACC,AGE_ACC,stored_model_path,epoch_nb):
    ...
    if model.task_type==4:
        ...
        for batch_nb in range(BATCH_NB_TEST_1):
            Image_data_1, Labels_1 = emotion_generater.__getitem__(batch_nb)
            emotion_loss = model.test_on_batch(Image_data_1,
                [Labels_1['emotion_prediction'],Labels_1['age_prediction']])
            loss = [emotion_loss[1],emotion_loss[3]]
            Losses_test_emotion.append(emotion_loss)
        for batch_nb in range(BATCH_NB_TEST_2):
            Image_data_2, Labels_2 = gender_age_generater.__getitem__(batch_nb)
            gender_age_loss = model.test_on_batch(Image_data_2,
                [Labels_2['emotion_prediction'],Labels_2['age_prediction']])
            loss2 = [gender_age_loss[2],gender_age_loss[4]]
            Losses_test_gender_age.append(gender_age_loss) 
        emotion_loss = np.array(Losses_test_emotion).mean(axis=0)[1]
        age_loss = np.array(Losses_test_gender_age).mean(axis=0)[2]
        total_loss = emotion_loss + age_loss
        emotion_acc = np.array(Losses_test_emotion).mean(axis=0)[3]
        age_acc = np.array(Losses_test_gender_age).mean(axis=0)[4]
        valid_result = [total_loss,emotion_loss,age_loss,emotion_acc,age_acc]
        valid_average_emotion_acc= round(emotion_acc,2)
        valid_average_age_acc= round(age_acc,2)
        EMOTION_ACC,GENDER_ACC,AGE_ACC = store_model_choice(model,EMOTION_ACC,GENDER_ACC,AGE_ACC,valid_average_emotion_acc,0,valid_average_age_acc,stored_model_path,epoch_nb)
...
def main():
    ...
    train_names_1=['train_total_loss','train_emotion_loss_1','train_gender_loss_1','train_age_loss_1',
    'train_emotion_acc_1','train_gender_acc_1','train_age_acc_1']
    train_names_2=['train_total_loss','train_emotion_loss_2','train_gender_loss_2','train_age_loss_2',
    'train_emotion_acc_2','train_gender_acc_2','train_age_acc_2']

    train_names=['train_total_loss','train_emotion_loss','train_age_loss',
    'train_emotion_acc','train_age_acc']
    train_names_22=['train_total_loss','train_emotion_loss_2','train_age_loss_2',
    'train_emotion_acc_2','train_age_acc_2']
    ...
    print('emotion_data:',len(emotion_train_idx),len(emotion_test_idx))
    print('gender_age_data:',len(gender_age_train_idx),len(gender_age_test_idx))
    ...
    log_path,stored_model_path = path_file(model,EMOTION_DATASET,GENDER_AGE_DATASET,MODEL)
    ...
    for epoch_nb in range(EPOCH):
        ...
        Losses_2 = []
        if epoch_nb!=0:
            emotion_model = freeze_all(model)
            emotion_model.compile(optimizer='adam', loss=losses,metrics=metrics)
        else:
            emotion_model = None
        emotion_generater = DataGenerator_emotion(model,emotion_model,train_emotion_paths,train_emotion, BATCH_SIZE)
        for i in range(2):
            for batch_nb in range(BATCH_NB_1):  
                ...
                if model.task_type==4:
                    ...
                    losses_1 = model.train_on_batch(Image_data_1,
                        [Labels_1['emotion_prediction'],Labels_1['age_prediction']])
                    Losses_1.append(losses_1)
            losses_data1 = np.array(Losses_1).mean(axis=0)
        
        #test
            emotion_test_generater = DataGenerator_emotion(model,emotion_model,test_emotion_paths,test_emotion, BATCH_SIZE)
            gender_age_test_generater = DataGenerator_gender_age(model,emotion_model,test_gender_age_paths,test_gender,test_age,BATCH_SIZE)
            ...
            write_log(callback,train_names,losses_data1,2*epoch_nb)
        ...
        if epoch_nb!=0:
            gender_age_model = freeze_all(model)
            gender_age_model.compile(optimizer='adam', loss=losses,metrics=metrics)
        else:
            gender_age_model = None
        gender_age_generater = DataGenerator_gender_age(model,gender_age_model,train_gender_age_paths,train_gender,train_age,BATCH_SIZE)
        for i in range(2):
            for batch_nb in range(BATCH_NB_2):
                [Image_data_2, Labels_2] = gender_age_generater.__getitem__(batch_nb)
                if model.task_type==4:
                    losses_2 = model.train_on_batch(Image_data_1,
                        [Labels_1['emotion_prediction'],Labels_1['gender_prediction'],Labels_1['age_prediction']])
                    ...
                    losses_2 = model.train_on_batch(Image_data_1,
                        [Labels_1['emotion_prediction'],Labels_1['age_prediction']])
                    Losses_2.append(losses_2)
            losses_data2 = np.array(Losses_2).mean(axis=0)

            #test
            emotion_test_generater = DataGenerator_emotion(model,gender_age_model,test_emotion_paths,test_emotion, BATCH_SIZE)
            gender_age_test_generater = DataGenerator_gender_age(model,gender_age_model,test_gender_age_paths,test_gender,test_age,BATCH_SIZE)
            valid_result,EMOTION_ACC,GENDER_ACC,AGE_ACC = test_model(model,callback,emotion_valid_name,gender_valid_name,age_valid_name,
                emotion_test_generater,gender_age_test_generater,BATCH_NB_TEST_1,BATCH_NB_TEST_2,EMOTION_ACC,GENDER_ACC,AGE_ACC,stored_model_path,epoch_nb)
            write_log(callback,train_names,losses_data2,2*epoch_nb+1)
