...
from utils.confusion_MTL.confusion_emotion_age_generator import DataGenerator as emotion_age_DataGenerator
...
def my_mae(y_true, y_pred):
    ...
    mae = K.mean(K.abs(K.sum(K.cast(K.arange(0, 101), dtype='float32') * y_pred, axis=1) -
                        K.sum(K.cast(K.arange(0, 101), dtype='float32') * y_true, axis=1)), axis=-1)
...
def path_file(EMOTION_DATASET,GENDER_AGE_DATASET,MODEL):
    logs_path = './train_logs/CONFUSION/{}-{}/{}/'.format(EMOTION_DATASET,GENDER_AGE_DATASET,MODEL)
...
def main():
    ...
    # BATCH_NB = np.min([BATCH_NB_1,BATCH_NB_2])
    BATCH_NB = int((len(train_emotion_paths)+len(train_gender_age_paths))/BATCH_SIZE)    
    BATCH_NB_test = int((len(test_emotion_paths)+len(test_gender_age_paths))/BATCH_SIZE)
    print(len(train_emotion_paths),len(train_gender_age_paths),BATCH_NB)
    ...
    AGE_ACC = 0
    if model.task_type == 3:
        generater = emotion_age_DataGenerator(model, train_emotion_paths,train_gender_age_paths, train_emotion, train_age,BATCH_SIZE)
        test_generater = emotion_age_DataGenerator(model, test_emotion_paths,test_gender_age_paths, test_emotion, test_age,BATCH_SIZE)
    else:
        generater = DataGenerator(model, train_emotion_paths,train_gender_age_paths, train_emotion, train_gender, train_age,BATCH_SIZE)
        test_generater = DataGenerator(model, test_emotion_paths,test_gender_age_paths, test_emotion, test_gender, test_age,BATCH_SIZE)
    ...
    for epoch_nb in range(EPOCH):
        ...
        Losses=[]
        Losses_test = []
        for batch_nb in range(BATCH_NB): 
            if generater.__getitem__(batch_nb) == None:
                break
            else:
                [Image_data, Labels] = generater.__getitem__(batch_nb)
                losses = model.train_on_batch(Image_data,
                    [Labels['emotion_prediction'],Labels['gender_prediction'],Labels['age_prediction']])
                Losses.append(losses)
        train_loss = np.array(Losses).mean(axis=0)
        write_log(callback,train_names,train_loss,epoch_nb)
        ...
        write_log(callback,valid_names,valid_result,epoch_nb)
        
        # for batch_nb in range(BATCH_NB_test): 
        #     if test_generater.__getitem__(batch_nb) == None:
        #         break
        #     else:
        #         [Image_data, Labels] = test_generater.__getitem__(batch_nb)
        #         losses = model.train_on_batch(Image_data,
        #             [Labels['emotion_prediction'],Labels['gender_prediction'],Labels['age_prediction']])
        #         # write_log(callback,train_names_1,losses,batch_nb)
        #         Losses_test.append(losses)
        # train_loss = np.array(Losses).mean(axis=0)
        # valid_loss = np.array(Losses_test).mean(axis=0)
        # emotion_acc = valid_loss[4]
        # gender_acc = valid_loss[5]
        # age_acc = valid_loss[6]
        # EMOTION_ACC,GENDER_ACC,AGE_ACC = store_model_choice(model,EMOTION_ACC,GENDER_ACC,AGE_ACC,emotion_acc,gender_acc,age_acc,stored_model_path,epoch_nb)

        
        # write_log(callback,valid_names,valid_loss,epoch_nb)
        print  ('Done for Epoch %d.'% epoch_nb)
        print  ('data_train_result:',train_loss)
