...
def cluster_curriculum_subsets(X, y, n_subsets=5, method='default', density_t=0.6, verbose=False,
    ...
    # reduce dimension to the specofied dimension
    ...
    for cluster_idx, current_category in enumerate(unique_categories):
        ...
        for batch_range in gen_batches(len(dist_list), batch_size=batch_max):
            ...
            # finding out the minimum density of cluster as bound 
            cluster_mins = [np.min(c) for c in clusters]
            # sorted bound, so dividing clean, less clean, least clean
...
class CurriculumClustering(BaseEstimator, ClusterMixin):
    ...
    def fit(self, X, y):
        X = check_array(X, accept_sparse='csr')
        check_consistent_length(X, y)
        self.output_labels, self.auxiliary = cluster_curriculum_subsets(X, y, **self.get_params())
        return self
