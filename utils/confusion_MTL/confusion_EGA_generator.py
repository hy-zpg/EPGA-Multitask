...
class DataGenerator(Sequence):
    ...
    def __len__(self):
        ...
        emotion_batch = int(self.batch_size*(emotion_length/(emotion_length+gender_age_length)))
        gender_age_batch = self.batch_size - emotion_batch
        length = np.min([int(emotion_length/emotion_batch),int(gender_age_length/gender_age_batch)])
        return length
    ...
    def __getitem__(self, idx: int):
        ...
        if self.model.task_type == 4:
            ...
            GENDER = np.concatenate([gender_fake1,Gender],axis=0)
